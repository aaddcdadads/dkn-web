  ...
  <div class="page-card">
    <div class="card-page" v-for="(item, index) in cardList" :key="index">
      <div class="container">
        ...
        <div class="time_card">
          <span class="time">{{creationTime}}: {{item.time}}</span>
          <span class="creator">{{creator}}: {{item.people}}</span>
        </div>
      </div>
...
export default {
  ...
  props: {
    ...
    img: {
      type: String,
      default: "https://block-design.oss-cn-shenzhen.aliyuncs.com/project-imgs/DatabaseDesign/sjkicon.png"
    },
    /**
     * 类型名
     */
  ...
  mounted() {
    if(!this.isType){this.height=170}
