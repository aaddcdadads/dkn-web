  ...
  <div class="page card">
    <div class="page-wrapper">
      ...
      <hm-panel ref="crudPagePanel" :height="crudPagePanel.height" :title="crudPagePanel.title" :padding="crudPagePanel.padding" :title-block-color="crudPagePanel.titleBlockColor" class="ele-crudPagePanel"> 
            <div class="ele-wrapper ele-wrapper-42e9cd13-e4ac-4d63-9746-3a6cf3a0fde9">
      <hm-ant-icon-text text="筛选查询" icon="fa fa-search" width="" font-size="14px" icon-size="14px" class="ele-42e9cd13-e4ac-4d63-9746-3a6cf3a0fde9"> 
       
    </hm-ant-icon-text>
    </div> 
            <div class="ele-wrapper ele-wrapper-activityName">
      <hm-ant-bg-text ref="activityName" :text="activityName.text" :font-size="activityName.fontSize"> 
       
    </hm-ant-bg-text>
    </div> 
            <div class="ele-wrapper ele-wrapper-registrationOrdersFilter">
      <hm-ant-formily ref="registrationOrdersFilter" :config="registrationOrdersFilter.config" v-model:value="registrationOrdersFilter.value" :col-num="5" :col-min-width="280" :schema="registrationOrdersFilter.schema" :label-col="7" :wrapper-col="14" class="ele-registrationOrdersFilter"> 
      ...
      <hm-ant-upload ref="importButton" title="" text="导入" :button-type="'primary'" v-model:file-list="importButton.fileList" action="/api/restify/registrationOrders/importExcel" :headers="importButton.headers" :visible="importButton.visible" class="ele-importButton"> 
      ...
      <hm-panel ref="activityPanel" :height="activityPanel.height" :title="activityPanel.title" :box-shadow-color="activityPanel.boxShadowColor" class="ele-activityPanel"> 
          <template v-slot:slot-title-icon>
        <div class="ele-wrapper ele-wrapper-numberUsers">
      <hm-bg-card ref="numberUsers" width="100%" height="30px" border-radius="" padding="" box-shadow-color="#00000000" class="ele-numberUsers"> 
      ...
      <hm-ant-button ref="bacthHeImportButton" :text="bacthHeImportButton.text" :type="bacthHeImportButton.type" :disabled="bacthHeImportButton.disabled" @click="onBacthHeImportButtonClick" class="ele-bacthHeImportButton"> 
       
    </hm-ant-button>
    </div> 
            <div class="ele-wrapper ele-wrapper-activitySelect">
      <hm-ant-select ref="activitySelect" v-model:value="activitySelect.value" url="/api/dkn/activity/list" :params="activitySelect.params" :data-map="activitySelect.dataMap" title="  " placeholder="选择活动" v-model:options="activitySelect.options" @change="onActivitySelectChange" class="ele-activitySelect"> 
       
    </hm-ant-select>
    </div> 
            <div class="ele-wrapper ele-wrapper-baoMingImportButton">
      <hm-ant-button ref="baoMingImportButton" text="导出" :type="'primary'" icon="fa fa-download" @click="onBaoMingImportButtonClick" class="ele-baoMingImportButton"> 
      ...
      <hm-ant-checkbox ref="quanXuanBox" title="" v-model:ccheckbox-list="quanXuanBox.ccheckboxList" @onChange="onQuanXuanBoxOnChange" class="ele-quanXuanBox"> 
       
    </hm-ant-checkbox>
    </div> 
            <div class="ele-wrapper ele-wrapper-actionSelect">
      <hm-ant-select ref="actionSelect" v-model:value="actionSelect.value" title="   " placeholder="批量操作" v-model:options="actionSelect.options" @change="onActionSelectChange" class="ele-actionSelect"> 
      ...
      <hm-modal ref="registrationOrdersAddModal" title="新增" v-model:visible="registrationOrdersAddModal.visible" width="810px" :z-index="1000" height="100%" :auto-close="false" @ok="onRegistrationOrdersAddModalOk" @cancel="onRegistrationOrdersAddModalCancel"> 
      ...
      <hm-ant-formily ref="registrationOrdersAddForm" :config="registrationOrdersAddForm.config" v-model:value="registrationOrdersAddForm.value" :col-num="2" :schema="registrationOrdersAddForm.schema" :label-col="7" :wrapper-col="14" class="ele-registrationOrdersAddForm"> 
      ...
      <hm-modal ref="registrationOrdersEditModal" title="编辑" v-model:visible="registrationOrdersEditModal.visible" width="810px" :z-index="1000" height="100%" :auto-close="false" @ok="onRegistrationOrdersEditModalOk" @cancel="onRegistrationOrdersEditModalCancel"> 
      ...
      <hm-ant-formily ref="registrationOrdersEditForm" :config="registrationOrdersEditForm.config" v-model:value="registrationOrdersEditForm.value" :col-num="2" :schema="registrationOrdersEditForm.schema" :label-col="7" :wrapper-col="14" class="ele-registrationOrdersEditForm"> 
      ...
      <hm-modal ref="registrationOrdersDetailModal" title="查看详情" v-model:visible="registrationOrdersDetailModal.visible" :ok-button-boole="false" cancel-text="关闭" width="810px" :z-index="1000" height="100%"> 
      ...
      <hm-ant-formily ref="registrationOrdersDetailForm" :config="registrationOrdersDetailForm.config" v-model:value="registrationOrdersDetailForm.value" :col-num="2" :schema="registrationOrdersDetailForm.schema" :label-col="7" :wrapper-col="14" class="ele-registrationOrdersDetailForm"> 
      ...
      <hm-modal ref="registrationOrdersDeleteModal" title="核销" v-model:visible="registrationOrdersDeleteModal.visible" :closable="true" ok-text="确认" :ok-type="'danger'" width="320px" :z-index="1000" height="80px" @ok="onRegistrationOrdersDeleteModalOk" @cancel="onRegistrationOrdersDeleteModalCancel"> 
      ...
      <hm-ant-icon-text ref="deleteUserInputText" text="是否确认核销?" icon="" width="200px" icon-size="0px" color="#CF2323" color-text="#1F1E1E" :text-align="'left'" padding="0" icon-padding="0" class="ele-deleteUserInputText"> 
...
export default {
  ...
  "registrationOrdersFilter": {
    "config": {
      ...
      "reset": {
        ...
        "events": {
          "@click": function(e) {self.$refs.registrationOrdersFilter.reset();
  ...
  if (this.$route.query.activityId) {
    ...
    self.activityPanel.title =
      `报名列表 (报名用户总数：${item.orders} 参与人次总数：${item.ordersTotal}）`
  ...
  self.setColumns = async function() {
    ...
    if (res.result.records.length > 0) {
      ...
      self.registrationOrdersTable.columns = [par, ...columns, ...self.registrationOrdersTable.columns]
    }
  }
  self.setColumns()
},
  ...
  let params = {
    ...self.$getFilterValues(self.$refs.registrationOrdersFilter.getFormValues(),
  ...
  if (this.selectTempArr.length > 0) {
    ...
    this.selectTempArr.forEach(e => {
      var startTimepick = this.$moment(e.pickUpStartTime, "YYYY-MM-DD HH:mm:ss");
      var endTimepick = this.$moment(e.pickUpEndTime, "YYYY-MM-DD HH:mm:ss");
