...
export default {
 ...
 //调用启用禁用方法
  this.isDisabled();
    ...
    async function downloadQrCodeAndShowStoreName() {
  try {
    ...
    downloadLink.style.display = 'none'; // 隐藏链接直到点击

    // 创建用于显示门店名称的p标签
    const storeNameParagraph = document.createElement("p");
    storeNameParagraph.textContent = "门店名称: " + this.storeNameTitle;

    // 将二维码和门店名称添加到DOM中
    const container = document.createElement("div"); // 创建一个容器来包含二维码和门店名称
    container.appendChild(downloadLink);
    container.appendChild(storeNameParagraph);
    document.body.appendChild(container); // 将容器添加到body中

    // 模拟点击链接来触发下载
    downloadLink.click();

    // 下载完成后，从DOM中移除a标签，但保留门店名称
    setTimeout(() => {
      document.body.removeChild(container); // 这将同时移除二维码和门店名称，如果你只想移除二维码，则需要单独处理
    }, 0);
