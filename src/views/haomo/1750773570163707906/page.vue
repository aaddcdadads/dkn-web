...
export default {
 ...
 //调用启用禁用方法
  this.isDisabled();
    ...
    async function downloadQrCodeAndStoreName() {
  try {
    // 确保baseUrl和storeNameTitle存在
    if (!this.baseUrl || !this.storeNameTitle) {
      throw new Error('Base URL or store name is not available.');
    }

    // 创建用于下载二维码的a标签
    const downloadLinkQrCode = document.createElement("a");
    downloadLinkQrCode.href = this.baseUrl;
    downloadLinkQrCode.download = this.storeNameTitle + "-二维码.png";
    downloadLinkQrCode.style.display = 'none'; // 隐藏链接

    // 创建用于下载门店名称的a标签
    const downloadLinkStoreName = document.createElement("a");
    const storeNameText = `Store Name: ${this.storeNameTitle}\n`;
    const blob = new Blob([storeNameText], { type: 'text/plain;charset=utf-8' });
    downloadLinkStoreName.href = URL.createObjectURL(blob);
    downloadLinkStoreName.download = this.storeNameTitle + "-门店名称.txt";
    downloadLinkStoreName.style.display = 'none'; // 隐藏链接

    // 将两个a标签添加到文档中
    document.body.appendChild(downloadLinkQrCode);
    document.body.appendChild(downloadLinkStoreName);

    // 模拟点击链接来触发下载
    downloadLinkQrCode.click();
    downloadLinkStoreName.click();

    // 下载完成后，从DOM中移除a标签
    setTimeout(() => {
      document.body.removeChild(downloadLinkQrCode);
      document.body.removeChild(downloadLinkStoreName);
    }, 0);

    // 如果需要，可以隐藏二维码模态框
    this.qrcodeModal.visible = false;
  } catch (error) {
    console.error('Error downloading QR code or store name:', error);
    // 在这里可以处理错误，比如显示一个错误消息给用户
  }
},
    onSureButtonClick () {
		//关闭弹窗
