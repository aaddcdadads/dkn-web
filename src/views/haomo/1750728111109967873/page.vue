<template>
  <div class="page card">
    <div class="page-wrapper">
      ...
      <hm-modal ref="registrationOrdersDeleteModal" title="核销" v-model:visible="registrationOrdersDeleteModal.visible" :closable="true" ok-text="确认" :ok-type="'danger'" width="320px" :z-index="1000" height="80px" :auto-close="false" @ok="onRegistrationOrdersDeleteModalOk" @cancel="onRegistrationOrdersDeleteModalCancel"> 
...
export default {
  ...
  "registrationOrdersTable": {
    ...
    "columns": [
      ...
      },
      {
        ...
        return self.processingData(item.record.pickUpStatusAction, item.record.paymentStatus, item.record.pickUpStartTime, item.record.pickUpEndTime);
     }
      }
    ],
    "data": [
  ...
  self.processingData = function(data, paymentStatus, startTime, endTime) {
    ...
    for (let i = 0; i < splData.length; i++) {
      ...
      var statusFlag = paymentStatus === 0 && splText[1] === 1 &&
        currentTimepick.isBetween(startTimepick, endTimepick) ?
        0 : 1;
      resultButton.push(h(HmAntButton, {
  ...
  if (this.selectTempArr.length > 0) {
    ...
    for (let i = 0; i < this.selectTempArr.length; i++) {
      var startTimepick = this.$moment(this.selectTempArr[i].pickUpStartTime, "YYYY-MM-DD HH:mm:ss");
      var endTimepick = this.$moment(this.selectTempArr[i].pickUpEndTime, "YYYY-MM-DD HH:mm:ss");
      if (!currentTimepick.isBetween(startTimepick, endTimepick)) {
